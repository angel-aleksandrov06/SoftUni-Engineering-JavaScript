function solve() {
    ...
    let fligthNumberReg = /  [A-Z]{1,3}[\d]{1,5}  /gm;
    ...
    let funcObj = {
        ...
        'flight': () => {
            ...
            return `Your flight number ${stringInput.match(fligthNumberReg)[0].trim()} is from ${from} to ${to}.`
        },
        'company': () => `Have a nice flight with ${stringInput.match(companyReg)[0].trim().replace("- ", "").replace("*", " ")}.`,
        'all': () => {
            let temp = stringInput.match(airportReg);
            let [from, to] = stringInput.match(airportReg)[0].trim().split("/");
            return `Mr/Ms, ${stringInput.match(nameReg)[0].trim().replace(/-/g, " ")}, your flight number ${stringInput.match(fligthNumberReg)[0].trim()} is from ${from} to ${to}. Have a nice flight with ${stringInput.match(companyReg)[0].trim().replace("- ", "").replace("*", " ")}.`
        }
    }

    result.innerText = funcObj[method.trim()]()
}